...
namespace SharpOnvifCommon.Xml
{
    ...
    public static class SoapEnvelope
    {
        ...
        /// body is empty. Throws <see cref="SoapFaultException"/> when the body carries a fault.
        /// </summary>
        public static bool MoveToBody(XmlReader reader)
        {
            if (!MoveToEnvelopeChild(reader, "Body"))
                throw new SoapFaultException("The response is not a SOAP envelope with a Body.");
            ...
            while (reader.Read())
            {
                ...
                if (reader.LocalName == "Fault" && reader.NamespaceURI == OnvifXmlNamespaces.SoapEnvelope)
                    throw SoapFaultReader.Read(reader).ToException();
